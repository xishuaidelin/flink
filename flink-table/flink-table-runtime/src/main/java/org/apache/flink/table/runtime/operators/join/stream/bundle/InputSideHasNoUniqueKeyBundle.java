...
public class InputSideHasNoUniqueKeyBundle implements BufferBundle {
    ...
    public int addRecord(RowData joinKey, @Nullable RowData uniqueKey, RowData record) {
        ...
        if (!foldRecord(joinKey, hashKey, record)) {
            bundle.get(joinKey).computeIfAbsent(hashKey, k -> new ArrayList<>()).add(record);
        }
        return ++count;
    }
    ...
    private boolean foldRecord(RowData joinKey, int hashKey, RowData record) {
        ListIterator<RowData> iterator =
                bundle.get(joinKey)
                        ...
                        .listIterator(bundle.get(joinKey).get(hashKey).size());
